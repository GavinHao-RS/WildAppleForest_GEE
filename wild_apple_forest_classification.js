...
- Cloud handling: Sentinel-2 QA60 mask + s2cloudless probability; composites use NDVI-based quality mosaics (no .median())
...
function maskClouds(image) {
  var cloudProb = ee.Image(image.get('cloud_probability')).select('probability');
...
function seasonalComposite(year, startMonth, endMonth) {
  ...
  var s2 = s2Sr.filterDate(start, end)
    .filterBounds(roi)
    .map(function(img) {
      var prob = s2Cloudless.filterBounds(img.geometry())
        .filterDate(img.date(), img.date().advance(1, 'day'))
        .first();
      return img.set('cloud_probability', prob);
    })
    .map(maskClouds)
